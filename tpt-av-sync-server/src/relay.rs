...
impl RelayServer {
        ...
        rt.spawn(async move {
            while !accept_inner.shutdown.load(Ordering::Relaxed) {
                ...
                tokio::spawn(async move {
                    ...
                    RelayServer::serve_connection(task_inner, ws).await;
                });
            }
        });
    ...
    async fn serve_connection<S>(inner: Arc<RelayInner>, ws: tokio_tungstenite::WebSocketStream<S>)
    where
        S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
    {
        ...
        while let Some(msg) = reader.next().await {
            ...
            let Ok(frame) = bincode::deserialize::<RelayFrame>(&bytes) else {
                continue;
            };
            ...
            match &frame.message {
                ...
                SyncMessage::RequestSnapshot => {
                    let stored = inner
                        ...
                        .unwrap_or_default();
                    if !stored.is_empty() {
                        if let Ok(reply_bytes) = bincode::serialize(&RelayFrame {
...
impl RelayClientTransport {
    /// Connects to the relay at `addr` and joins `room`.
    pub fn connect(
        addr: SocketAddr,
        ...
        // Join the room (fan-out frame; also serves as presence).
        let join_bytes = bincode::serialize(&RelayFrame {
            room: room.clone(),
            from: local,
            to: None,
                    ...
                    maybe_in = reader.next() => {
                        match maybe_in {
                            Some(Ok(WsMessage::Binary(bytes))) => {
                                if let Ok(frame) = bincode::deserialize::<RelayFrame>(&bytes) {
