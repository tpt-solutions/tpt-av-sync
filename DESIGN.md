...
- [x] Long-run soak: 24 h simulated multi-peer session in CI: `tpt-av-sync-net/tests/soak.rs` — 5 peers over an in-memory mesh, 1-minute simulated ticks for 24 simulated hours (~7,000 operations from a deterministic PRNG), presence aging, periodic `TimelineCrdt::compact()`, and convergence checks every 6 simulated hours plus a final one. Runs in ~0.2s of wall-clock time (no real sleeps — only the simulated timestamp advances), so it runs on every `cargo test --workspace` in CI rather than needing a separate long-running job.
