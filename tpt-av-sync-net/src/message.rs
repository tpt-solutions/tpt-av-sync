...
/// wire compatibility; peers with mismatched versions should be rejected by
/// transports.
...
mod tests {
    ...
    fn every_message_variant_roundtrips() {
        ...
        for msg in messages {
            ...
            let back: SyncMessage = bincode::deserialize(&bytes).unwrap();
            assert_eq!(back, msg);
        }
    }
    ...
    fn wire_frame_roundtrips() {
        ...
        let back: WireFrame = bincode::deserialize(&bytes).unwrap();
