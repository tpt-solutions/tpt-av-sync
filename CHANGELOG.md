...
- `tpt-av-sync-net/tests/soak.rs`: a 24-simulated-hour, 5-peer multi-peer session soak test (deterministic PRNG, ~7,000 operations issued in total, presence aging, periodic compaction, staged convergence checks) — runs in ~0.2s of wall-clock time as part of the normal test suite. Hourly compaction kept each peer's final operation log at ~284 entries despite ~7,000 operations having been issued over the session.
